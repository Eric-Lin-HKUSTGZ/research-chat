...
TEST_DIR="$REPO_ROOT/backend/tests/unit"
...
# Make sure Python can import backend/app package
...
if [[ "$HAVE_PYTEST_COV" == "1" ]]; then
  ...
  PYTEST_ARGS+=( -p pytest_cov --cov=backend --cov-report=term-missing --cov-report=xml:"$COV_XML" --cov-report=html:"$COV_HTML_DIR" )
  if [[ -n "$COV_MIN" ]]; then
